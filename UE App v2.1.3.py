...
def submit(prefix1, prefix2):
    ...
    get_details(driver)      
    team_selection(driver, players, positions, gks, roles_df)
...
def team_selection(driver, players, positions, gks, roles_df):
...
def create_team_layout(team_layout, gks, player_prev, deff, deff_pos, pos_prev, mid, mid_pos, att, att_pos, FRY):
    ...
    row_configs = [
        (gks, ['GK'], "1", player_prev[0], None),
        (FRY, positions, "2", player_prev[1], pos_prev[1]),
        (FRY, positions, "3", player_prev[2], pos_prev[2]),
        (FRY, positions, "4", player_prev[3], pos_prev[3]),
        (FRY, positions, "5", player_prev[4], pos_prev[4]),
        (FRY, positions, "6", player_prev[5], pos_prev[5]),
        (FRY, positions, "7", player_prev[6], pos_prev[6]),
        (FRY, positions, "8", player_prev[7], pos_prev[7]),
        (FRY, positions, "9", player_prev[8], pos_prev[8]),
        (FRY, positions, "10", player_prev[9], pos_prev[9]),
        (FRY, positions, "11", player_prev[10], pos_prev[10]),
...
def read_sql_data(prefix, prefix2): # Connect to the database and load the player names
    ...
    return gks, deff, mid, att, deff_pos, mid_pos, att_pos, player_prev, pos_prev, FRY

def write_to_database(team_boxes, prefix2, tactic_boxes, role_boxes, training_boxes, substitute_options, action_boxes):
...
def ry_action_selection(action_layout, whole_team, prefix2, FRY):
    ...
    for i in range(num_rows):
        ...
        player_combo_box = QComboBox()
        player_combo_box.addItems(FRY)  # Add full list of players
        if current_player and current_player in player_items:
            player_combo_box.setCurrentText(current_player)  # Set previous player if available
        player_combo_box.setFixedWidth(100)
        ...
        player_completer = SubstringCompleter(player_items)
...
def set_substitute_options(substitute_layout, whole_team, prefix2, FRY):
    ...
    for i in range(num_rows): # Define rows with identifiers and populate substitute_layout
        ...
        row, minute_box, circumstance_box, player_on_box, player_off_box, sub_position_box = create_substitute_options( #retrieve row and the 5x boxes from create substitute options function
            circumstances, whole_team, positions, row_id, prev_minute[i], prev_circumstance[i], prev_player_on[i], prev_player_off[i], prev_sub_position[i], FRY # send variables to create substitute option function | last 5 are for setting the boxes 
        )
...
def create_substitute_options(circumstances, whole_team, positions, row_id, prev_minute, prev_circumstance, prev_player_on, prev_player_off, prev_sub_position, FRY):
    row = QHBoxLayout()  # Create a new horizontal layout for each row
    row.setAlignment(Qt.AlignmentFlag.AlignTop | Qt.AlignmentFlag.AlignLeft)
    ...
    return row, minute_box, circumstance_box, player_on_box, player_off_box, sub_position_box

def setup_team_layout(self, gks, deff, mid, att, player_prev, deff_pos, pos_prev, mid_pos, att_pos, prefix, prefix2, FRY):
    # Define whole team
    ...
    combo_boxes, positional_value_labels = create_team_layout(team_layout, gks, player_prev, deff, deff_pos, pos_prev, mid, mid_pos, att, att_pos, FRY)
    ...
    if prefix2 == "cup_":
        ...
        def update_cup_roles():
            if cup_roles_check.isChecked():
                conn = sqlite3.connect("UE.db")
                cup_roles = pd.read_sql("SELECT * FROM ft_roles",conn)
                conn.close()
                print(cup_roles)
        ...
        def update_cup_tactics():
            if cup_tactics_check.isChecked():
                conn = sqlite3.connect("UE.db")
                cup_tactics = pd.read_sql("SELECT * FROM ft_tactics",conn)
                conn.close()
                print(cup_tactics)
        ...
        def update_cup_subs():
            if cup_subs_check.isChecked():
                conn = sqlite3.connect("UE.db")
                cup_subs = pd.read_sql("SELECT * FROM ft_subs",conn)
                conn.close()
                print(cup_subs)
    ...
    if prefix2 =="cup_":
        button_write.clicked.connect(lambda: write_to_database(combo_boxes, prefix2, tactic_boxes, role_boxes, training_boxes, substitute_boxes, action_boxes=None))
    else:
        button_write.clicked.connect(lambda: write_to_database(combo_boxes, prefix2, tactic_boxes, role_boxes, training_boxes, substitute_boxes, action_boxes))
...
class Cup(QWidget):
    def __init__(self, parent=None):
        ...
        gks, deff, mid, att, deff_pos, mid_pos, att_pos, player_prev, pos_prev, FRY = read_sql_data(prefix, prefix2)

        setup_team_layout(self, gks, deff, mid, att, player_prev, deff_pos, pos_prev, mid_pos, att_pos, prefix, prefix2, FRY)
...
class MDSDataWorker(QThread):
    ...
    def run(self):
        try:
            self.progress_update.emit(10)  # Example: Start progress

            # Call your data processing functions here
            mds_data = get_MDS_submission_data(self.file)
            self.progress_update.emit(20)  # Update progress
            ...
            self.progress_update.emit(50)  # Midway progress
            ...
            self.progress_update.emit(80)  # Near completion

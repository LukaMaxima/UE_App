...
def get_MDS_submission_data(file):
    ...
    search_string = 'William Elliott Sports Ltd'
...
def get_reserve_team_data(file):
    search_string = r'\bReserve Team\b(?!\s*(?:Coach|Manager))'  # Regex pattern to match "Reserve Team" as an exact phrase
    pdf_file = open(file, 'rb')
    pdf_reader = PyPDF2.PdfReader(pdf_file)
...
def get_youth_team_data(file):
    search_string = r'Youth Team\b(?!\s*(?:Coach|Manager))' # Regex pattern to match "Reserve Team" as an exact phrase
    pdf_file = open(file, 'rb')
    pdf_reader = PyPDF2.PdfReader(pdf_file)
...
def match_reports(file):
    ...
    if not CHECK_FOLDER:
        os.makedirs(MR_DIR)
        print("\ncreated folder : ", MR_DIR)
    else:
        print(MR_DIR, "\nfolder already exists.")

    print("\nGathering awesome player scores... please standby for raw epicness")
